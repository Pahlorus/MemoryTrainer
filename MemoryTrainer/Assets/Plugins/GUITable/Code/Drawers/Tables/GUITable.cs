...
namespace EditorGUITable
{
	...
	public static class GUITable
	{
			...
			params GUITableOption[] options) 
		{
			...
			int i = 0;
			if (tableEntry.showIndex)
            {
				columns.Insert(0, new SelectFromFunctionColumn((x,index) => new LabelCell(i.ToString()), TableColumn.EnabledCells(false), TableColumn.Title("Index"), TableColumn.EnabledTitle(false)));
            }
			for ( ; i < collectionProperty.arraySize ; i++)
			{
